...
class ZoneCat(object):
    ...
    def __init__(self, gdb, ras, lyrName=None, fldname=None, projIn=None,
                 projOut=None, buffDist=0, fact=30, outND=np.nan, nd_thresh=100,
                 filenm='outputfile', csvout=False, cmap=None):
        self.gdb = gdb
        self.ras = ras
    ...
    def compute_category(self):
        ...
        if self.projIn == None:
            self.projIn = vlyr.GetSpatialRef().ExportToProj4()
        ...
        for i, feat in enumerate(vlyr):
            if self.fldname is None:
                ...
                fldid = feat.GetField(self.fldname)
            ...
            if src_offset[2] <= 0 or src_offset[3] <=0:
                ...
                if vec_area / ras_area >= self.fact:
                    ...
                    zooms = int(np.sqrt(ras_area / (vec_area/self.fact)))
